...
/** Renders a technical article card — internal links use <A>, external use <a>. */
function ArticleCard(props: { article: Article; onNavigate: () => void }) {
  const isInternal = () => props.article.url?.startsWith("/");
...
/** Renders a reflection archive row — plain div (reflections have no destination URL). */
function ArchiveRow(props: { article: Article; isLast: boolean }) {
  const rowStyle = (): JSX.CSSProperties => (props.isLast ? s.archiveItemLast : s.archiveItem);
  return (
    <div id={props.article.slug} style={rowStyle()}>
      {archiveRowContent(props.article)}
    </div>
  );
}
        ...
        <div style={s.archiveDivider}>
          ...
          <div style={s.archiveList}>
            <For each={reflectionArticles}>
              {(article, i) => (
                <ArchiveRow article={article} isLast={i() === reflectionArticles.length - 1} />
              )}
            </For>
          </div>
        </div>
