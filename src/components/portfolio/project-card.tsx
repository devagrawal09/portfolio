...
export const ProjectCard = ({ project, allFeatures, allTools, i }: CardProps) => {
  const tools = project.tools.map((tool, i) => {
  ...
  const features = project.features.map((feature, i) => {
    ...
    return <FeatureBadge key={i} feature={featureData} projectId={project.id} />
  })
  ...
  const col1 = <Col xs={6}>
    ...
    >
      ...
      <GatsbyImage image={image} alt={project.name} className="project-image" />
    </Link>
  </Col>
